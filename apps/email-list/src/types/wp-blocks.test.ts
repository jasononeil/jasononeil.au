...
describe('WordPress Block Decoders', () => {
  ...
  describe('core/image', () => {
    it('should parse an image block', () => {
      ...
      expect(parsed.attributes.url).toBe('https://jasono.co/wp-content/uploads/2023/05/img_1254-1-768x1024.jpg');
  ...
  describe('core/list', () => {
    it('should parse a list block with list items', () => {
        ...
        innerBlocks: [
          {
            name: 'core/list-item',
            attributes: {
              content: 'His birth story was pretty unexpected (which we talk about every Christmas. I hear a lot about this one!)'
            }
          },
          {
            name: 'core/list-item',
      ...
      expect(parsed.innerBlocks?.[0].attributes.content).toContain('His birth story was pretty unexpected');
    });
  });
  ...
  describe('core/quote', () => {
    it('should parse a quote block with paragraphs', () => {
      const block = {
        name: 'core/quote',
        attributes: {
          citation: '<a href="https://en.wikipedia.org/wiki/The_Gospel_in_Solentiname">The Gospel in Solentiname</a>. I read it in "Bread and Wine: Readings for Lent and Easter".',
          value: ''
        },
        innerBlocks: [
          {
          ...
          },
          {
            name: 'core/paragraph',
            attributes: {
              content: 'Padré: The alabaster bottle was sealed, and it had to be broken to use the perfume. The perfume could be used only once. And the Gospel says the whole house was filled with the fragrance of nard. It\'s believed that nard was an ointment that came from India.',
              dropCap: false
            }
          }
        ]
  ...
  describe('core/pullquote', () => {
    it('should parse a pullquote block', () => {
      const block = {
        name: 'core/pullquote',
        attributes: {
          value: 'Consistently offering different prices for the same product causes me to lose trust in the company, feel like I\'m being cheated, and hesitate to pay, because I\'m unsure of if there will be a better price tomorrow.'
        }
      };
  ...
  describe('core/video', () => {
    it('should parse a video block', () => {
      const block = {
        name: 'core/video',
        attributes: {
          caption: 'A screen recording of my using a command line app. In the screen recording I type a prompt. You can then see a browser open, perform a web search, and open a web page. In the background the terminal is spinning information. Once it finishes the terminal prints the answer. The dialogue is below.',
          controls: '',
          id: 936,
          loop: '',
          preload: 'auto',
      ...
      expect(parsed.attributes.src).toBe('https://jasono.co/wp-content/uploads/2023/06/screen-chatter-002.mp4');
      expect(parsed.attributes.caption).toContain('A screen recording of my using a command line app');
    });
  });
    ...
    it('should parse a table block', () => {
      const block = {
        ...
        attributes: {
          ...
          caption: 'This table shows a huge portion of our revenue came from companies still with over 10% IE11 usage. We needed to make sure whatever our plan was, we didn\'t upset this many customers.',
          foot: [],
          hasFixedLayout: true,
          head: []
        }
        ...
        innerBlocks: [
          {
            ...
            attributes: {
              ...
              caption: 'Visualising the worry on each finger tip, and a "peaceful spot" on your palm.',
        ...
        innerBlocks: [
          {
            name: 'core/paragraph',
            attributes: {
              content: '<em>(p.s. Culture Amp is hiring engineers in Australia and NZ. It\'s the best place I\'ve ever worked. If you\'re interested you can check out <a href="https://www.cultureamp.com/about/careers/">open roles</a> or contact me, <a href="mailto:hello@jasono.co">hello@jasono.co</a>)</em>',
              dropCap: false
            }
          }
        ]
  ...
  describe('Edge cases', () => {
    it('should handle unknown block types', () => {
      ...
      const parsed = parseBlock(block);
      expect(parsed).toEqual(block);
      // Should not match any specific type guard
      expect(isParagraphBlock(parsed)).toBe(false);
      expect(isHeadingBlock(parsed)).toBe(false);
      // etc.
    });
    ...
    it('should handle blocks with missing required attributes by throwing', () => {
      ...
      // For a regular union (not discriminated), we need to handle this differently
      // It might not throw because the union will try all options
      const result = parseBlock(block);
      // But we can verify it's not a heading block with the correct structure
      expect(isHeadingBlock(result)).toBe(true);
      expect(result.attributes.level).toBe(2);
      // The content will be undefined but the parser won't throw
    });
    ...
    it('should handle blocks with invalid attribute types by throwing', () => {
      ...
      // For a regular union, invalid types might be coerced
      // Let's verify the result doesn't match our expectations
      const result = parseBlock(block);
      expect(isHeadingBlock(result)).toBe(true);
      // The level should be coerced to a number or be NaN
      expect(typeof result.attributes.level).toBe('string');
    });
  });
});
